...
`Python/Analyze` 폴더는 웹에서 수집한 재무제표 이미지를 분석하여 구조화된 데이터(CSV)로 변환하는 고성능 OCR 파이프라인입니다.

- **주요 라이브러리**: `easyocr`, `opencv-python`, `selenium`, `numpy`
- **분석 흐름**:
  1.  **이미지 수집 (`OCR_C.py`)**: `selenium`을 사용하여 웹 브라우저를 자동화하고, 네이버 금융에서 재무제표 테이블을 정밀하게 스크린샷하여 이미지 파일로 저장합니다.
  2.  **이미지 전처리 (`OCR_M.py`)**: HSV 색 공간을 활용하여 적색 및 흑색 텍스트를 효과적으로 분리하고 강조함으로써 OCR 인식률을 극대화합니다.
  3.  **OCR 수행 (`OCR_M.py`)**: `easyocr`을 사용하여 전처리된 이미지에서 텍스트와 위치 정보를 추출합니다.
  4.  **텍스트 후처리 및 구조화 (`OCR_M.py`)**:
      - `OCR_D.py`의 단어 사전을 이용해 금융 용어 오타를 교정합니다.
      - 텍스트의 공간적 배열을 분석하여 동적으로 테이블 구조(행, 열)를 복원합니다.
      - 숫자와 레이블이 붙어있는 오류 등 특정 패턴을 감지하고 수정하는 후처리 규칙을 적용하여 데이터 정확도를 높입니다.
...
1.  **`OCR_C.py`**: 웹 스크래핑을 통해 재무제표 이미지를 수집하는 스크립트입니다.
2.  **`OCR_M.py`**: 이미지 전처리, OCR, 테이블 구조화, 후처리 등 전체 분석 프로세스를 관장하고 최종 CSV 파일을 생성하는 메인 스크립트입니다.
3.  **`OCR_D.py`**: 자주 발생하는 OCR 한글 오타를 교정하기 위한 단어 딕셔너리를 제공하는 헬퍼 모듈입니다.
...
먼저 `OCR_C.py`를 실행하여 분석할 재무제표 이미지를 수집합니다. 이 스크립트는 `top_movers_auto.json` 파일에 정의된 종목들을 대상으로 이미지를 생성하여 `Python/Analyze/Asset/Img/` 폴더에 저장합니다.
*(현재 `OCR_C.py`의 입력 파일 경로는 하드코딩되어 있어, 필요시 수정이 필요할 수 있습니다.)*

```bash
python Python/Analyze/OCR_C.py
```

**1단계: OCR 분석**

`OCR_M.py`를 실행하여 이미지를 분석하고 CSV로 변환합니다. 이 스크립트는 `Python/Analyze/Asset/TestImg/` 폴더 내의 모든 이미지 파일을 자동으로 감지하여 순차적으로 처리합니다.

> **테스트 시 참고**: `OCR_C.py`의 결과물이 `Img` 폴더에 저장되므로, 분석을 위해서는 해당 이미지들을 `TestImg` 폴더로 옮기거나 `OCR_M.py`의 입력 경로를 수정해야 합니다. 현재 로직은 `TestImg` 폴더를 사용하도록 설정되어 있습니다.
...
예를 들어, `005930_financials.png`를 처리했다면 `005930_financials.csv` 파일이 생성됩니다.
