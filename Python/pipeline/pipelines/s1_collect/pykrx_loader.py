...
try:
    ...
    raise ImportError(
        "pykrx package is required. Install it with `pip install pykrx pandas`."
    ) from exc
...
try:
    ...
    raise ImportError(
        "pandas package is required. Install it with `pip install pandas`."
    ) from exc
...
def _save_dataframe(df: "pd.DataFrame", path: Path, *, index_name: str) -> Path:
    ...
    if index_name in df.columns:
        try:
            ...
            LOGGER.debug("failed to normalise datetime column %s", index_name, exc_info=True)
    ...
    LOGGER.info("saved pykrx output to %s", path)
    return path
