...
def _parse_dart_multi(path: Path, ticker: str) -> Optional[pd.DataFrame]:
    ...
    for entry in payload:
        ...
        pivot = values.pivot_table(index="date", columns="account_nm", values="value", aggfunc="last")
        pivot.columns = [f"fund_{_slugify(col)}" for col in pivot.columns]
        frames.append(pivot)
    if not frames:
        return None
    ...
    merged = merged.groupby(level=0).last()
